...
export function AuthGate({ children }: { children: React.ReactNode }) {
  ...
  async function submitInvite(event: React.FormEvent) {
    ...
    try {
      ...
      setInviteEmail(''); await loadInvites(); setMessage('Invitation email sent and beta access opened for that address.');
  ...
  async function resendInvite(inviteId: number) {
    ...
    try {
      await apiJson('/api/profile?auth_action=invite', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'resend', invite_id: inviteId }) });
      await loadInvites(); setMessage('Invitation email resent.');
    } catch (error) { setMessage(error instanceof Error ? error.message : 'Could not resend invitation'); }
    finally { setWorking(false); }
  }
  ...
  async function removeInvite(inviteId: number) {
    ...
    try {
      ...
      await loadInvites(); setMessage('Pending invitation removed.');
    } catch (error) { setMessage(error instanceof Error ? error.message : 'Could not remove invitation'); }
    finally { setWorking(false); }
  }
  ...
  if (panel === 'beta' && status?.is_owner) return <Shell><UserPlus className="w-12 h-12 text-gold mx-auto mb-4" /><h1 className="text-2xl font-serif text-white text-center mb-3">Private beta invites</h1><p className="text-white/50 text-sm text-center mb-5">Send an invitation email. Pending invites can be resent or removed at any time.</p><form onSubmit={submitInvite} className="space-y-3"><input required type="email" value={inviteEmail} onChange={e => setInviteEmail(e.target.value)} placeholder="friend@example.com" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white" /><button disabled={working} className="w-full bg-gold text-deep-blue font-bold py-3 rounded-xl">Send beta invite</button></form>{message && <p className="text-xs text-white/60 mt-4 text-center">{message}</p>}<div className="mt-6 max-h-64 overflow-y-auto space-y-2">{invites.map(invite => { const joined = Boolean(invite.accepted_at); const removed = Boolean(invite.revoked_at); return <div key={invite.id} className="rounded-xl border border-white/10 p-3"><div className="text-xs text-white/70 break-all">{invite.email}</div><div className="text-[10px] uppercase tracking-widest text-white/30 mt-1">{joined ? 'Joined' : removed ? 'Removed' : 'Pending'}</div>{!joined && <div className="flex gap-2 mt-3"><button disabled={working} onClick={() => resendInvite(invite.id)} className="flex-1 rounded-lg border border-gold/20 px-3 py-2 text-[10px] uppercase tracking-widest text-gold/70 hover:bg-gold/10 disabled:opacity-40">{removed ? 'Re-send & restore' : 'Resend'}</button>{!removed && <button disabled={working} onClick={() => removeInvite(invite.id)} className="flex-1 rounded-lg border border-red-500/20 px-3 py-2 text-[10px] uppercase tracking-widest text-red-300/70 hover:bg-red-500/10 disabled:opacity-40">Remove</button>}</div>}</div>; })}</div><button onClick={() => { setPanel(null); setMessage(''); }} className="w-full text-xs text-white/40 mt-5">Back to journal</button></Shell>;
